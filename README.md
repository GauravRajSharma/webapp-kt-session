...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create webapp-ce66
  fly apps create webapp-ce66-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app webapp-ce66
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app webapp-ce66-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'webapp-ce66-staging' does not match app name in config file 'webapp-ce66'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name webapp-ce66-db
  fly postgres attach --app webapp-ce66 webapp-ce66-db

  fly postgres create --name webapp-ce66-staging-db
  fly postgres attach --app webapp-ce66-staging webapp-ce66-staging-db
  ```
